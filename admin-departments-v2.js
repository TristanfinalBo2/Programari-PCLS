...
function styleDepartmentPanel() {
  ...
  if (rows.length !== 4) return host.classList.contains("pcls-dept-v2-grid");
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .pcls-dept-v2-card{
      ...
      background:linear-gradient(145deg,rgba(27,37,57,.88),rgba(11,17,30,.74)) !important;
      box-shadow:inset 0 1px 0 rgba(255,255,255,.035),0 12px 25px rgba(0,0,0,.12) !important;
      overflow:hidden !important;
      display:flex !important;
      flex-direction:column !important;
    ...
    @media(max-width:700px){
      .pcls-dept-v2-panel{height:auto !important;min-height:0 !important}
      .pcls-dept-v2-grid{grid-template-columns:1fr !important;grid-template-rows:repeat(4,94px) !important;height:auto !important;min-height:0 !important;flex:none !important}
    }
  `;
...
function boot() {
  ...
  if (!departmentObserver) {
    departmentObserver = new MutationObserver(() => {
      if (!styling) styleDepartmentPanel();
    });
    departmentObserver.observe(document.body, { childList:true, subtree:true });
  }
