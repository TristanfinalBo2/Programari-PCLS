...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!ALLOWED_ROLES.has(profile.role)) {
      return sendJson(res, 403, { ok: false, error: "Nu ai permisiunea de a trimite mesaje Discord din Admin Panel." });
    }
    ...
    return sendJson(res, 200, { ok: true, department, uid, roleId });
  } catch (error) {
