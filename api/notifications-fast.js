...
async function queryField(token, fieldPath, value, limit = 50) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const [byDiscord, byRecipientId] = await Promise.all([
      queryField(token, 'recipientDiscordId', discordId, 50),
      queryField(token, 'recipientId', discordId, 50)
    ]);
    const merged = new Map();
    [...byDiscord, ...byRecipientId].forEach(item => merged.set(item.id, item));
    let items = Array.from(merged.values());
    if (!items.length && session.email) items = await queryField(token, 'recipientEmail', String(session.email), 50);
    items.sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));

    if (req.method === 'POST') {
      const body = typeof req.body === 'object' && req.body ? req.body : {};
      if (body.action === 'readAll') {
        await Promise.all(items.filter(x => x.read !== true).map(x => patchNotification(token, x.id, { read: true })));
        return json(res, 200, { ok: true });
      }
      const id = String(body.id || '');
      if (!id) return json(res, 400, { ok: false, error: 'ID notificare lipsă.' });
      if (!items.some(x => x.id === id)) return json(res, 404, { ok: false, error: 'Notificarea nu există.' });
    ...
    console.error('notifications-fast error', error);
    return json(res, 500, { ok: false, error: error?.message || 'Eroare notificări.' });
  }
};
