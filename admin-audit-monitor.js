...
async function writeAudit(key, data) {
  try {
    ...
    console.warn("Audit log nu a putut fi salvat:", error?.message || error);
  }
}
...
function actionForButton(button) {
  ...
  if (classes.contains("btn-trash")) return "Cerere mutată/ștearsă";
  return null;
}
...
async function auditAdminAction(button) {
  ...
  // Handlerul existent din admin.html modifică mai întâi cererea.
  await new Promise(resolve => setTimeout(resolve, 350));

  try {
    const snap = await getDoc(doc(db, "cereri", targetId));
    const data = snap.exists() ? snap.data() || {} : {};
    const a = await actor();
    await writeAudit(`admin_action_${targetId}_${hash(`${action}|${Date.now()}|${a.uid}`)}`, {
      actorId: a.uid,
      actorName: a.name,
      actorRole: a.role,
      action,
    ...
    console.warn("Nu s-a putut înregistra acțiunea admin:", error?.message || error);
  }
}
...
function startRequests() {
  onSnapshot(collection(db, "cereri"), async snap => {
    ...
    initialized = true;
  }, error => console.warn("Monitor audit cereri:", error?.message || error));
}
...
function startUsers() {
  ...
  }, error => console.warn("Monitor audit utilizatori:", error?.message || error));
}
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => {
    if (document.getElementById("cereri-container") || location.pathname.toLowerCase().endsWith("admin.html")) init();
  }, { once: true });
} else if (document.getElementById("cereri-container") || location.pathname.toLowerCase().endsWith("admin.html")) {
  init();
}
