...
// Aceste 3 tipuri folosesc modalul clasic cu datele principale.
// Mesajul Discord este trimis automat la confirmare, dar NU este afișat în popup.
...
function isSimpleApprovalCase(o){
  ...
  return text.includes("inregistrare") || text.includes("programare") || text.includes("eveniment") || text.includes("event");
}
...
async function confirm(e){
  ...
  try{
    ...
    const content=simple ? buildMessage(latest) : (ta?.value?.trim()||buildMessage(latest));
    // Pentru înregistrare / programare / eveniment, Discord este trimis în fundal.
    await send(latest,content);
    if(ok)ok.textContent="✓ Mesajul a fost trimis.";
    await approve(latest);
    setTimeout(()=>{document.getElementById("approve-modal")?.classList.remove("active");restoreClassicSummary();selected=null;},350);
  }catch(err){
    console.error("Approval dispatch:",err);
    if(simple){
      // Pentru modalul clasic afișăm doar eroarea, fără preview Discord.
...
function bind(){
  ...
  document.addEventListener("click",async e=>{
    ...
    try{
      ...
      if(p){p.querySelector("#approval-dispatch-text").value=buildMessage(selected);p.querySelector("#approval-dispatch-error").textContent="";p.querySelector("#approval-dispatch-ok").textContent="";}
    }catch(err){console.error("Approval preview:",err);}
  },true);
  document.addEventListener("click",e=>{if(e.target.closest("#approve-ok-btn"))void confirm(e)},true);
  document.addEventListener("click",e=>{if(e.target.closest("#approve-cancel-btn")||e.target.closest("#approve-close")){selected=null;busy=false;restoreClassicSummary();}},true);
}
