...
export async function notifyRequestStatus(item, newStatus, actorName = "Administrator") {
  ...
  if (["aprobat", "acceptat", "approved"].includes(normalized)) {
    ...
    message = "Cererea ta a fost aprobată. Deschide notificarea pentru detalii.";
  } else if (["respins", "respinsa", "rejected"].includes(normalized)) {
    type = "error";
    title = "Cerere respinsă";
    message = "Cererea ta a fost respinsă. Verifică detaliile și motivul afișat în cerere.";
...
function mountCenter(container) {
  ...
  const toggle = container.querySelector("#notificationToggle");
  const dropdown = container.querySelector("#notificationDropdown");
...
function boot() {
  const container = document.getElementById("notification-section-premium");
  if (container) mountCenter(container);
}
